$(function () {
    ...
    $('#add-profile-form').bootstrapValidator({
        ...
        submitHandler: function (validator, form, submitButton) {
            ...
            $.ajax({
                url: 'http://localhost:8080/v1/user/' + screenName,
                type: "get",
                dataType: "json",
    ...
    $.ajax({
        url: 'http://localhost:8080/users/search/findRankedUsers?skip=0&limit=100',
        type: "get",
        dataType: "json",
        success: function (data) {
            $(".loader").hide();
            drawTable(data._embedded.user);
            drawDashboardHeader(data._embedded.user)
        },
        error: function (xhr, status) {
            $(".glyphicon-refresh")
