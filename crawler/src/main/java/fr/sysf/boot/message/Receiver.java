package fr.sysf.boot.message;

import com.fasterxml.jackson.databind.ObjectMapper;
import fr.sysf.boot.dao.FollowsRepository;
import fr.sysf.boot.dao.UserRepository;
import fr.sysf.boot.model.Follows;
import fr.sysf.boot.model.User;
import fr.sysf.boot.scheduling.AnalyticsScheduler;
...
public class Receiver {
    ...
    private void saveFollowers(User user, CursoredList<Long> followers) {
        ...
        List<User> users = followers.stream().map(a -> new User(
                a,
                Collections.singletonList(new User(finalUser.getId(), finalUser.getProfileId())).stream().collect(Collectors.toList())
                , null))
        ...
        while ((batchSize * pointer) < users.size()) {
            try {
                followsRepository.saveFollows(
                        users.subList((batchSize * pointer),
                                ((batchSize * pointer) + batchSize) < users.size() ? ((batchSize * pointer) + batchSize) : users.size()).stream()
